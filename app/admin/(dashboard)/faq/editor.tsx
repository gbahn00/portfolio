...
function Card({ item, onSaved, onDeleted }: { item: FaqItem; onSaved: (v: FaqItem) => void; onDeleted: () => void }) {
  ...
  async function save() {
    setSaving(true);
    try {
      const updated = await api(`/api/admin/list/faq/${item.id}`, { method: "PATCH", body: JSON.stringify(draft) });
      onSaved(updated);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="rounded-md border border-neutral-800 p-4 mb-3">
      <TextField label="질문" value={draft.question} onChange={(v) => set("question", v)} />
      <TextAreaField label="답변" value={draft.answer} onChange={(v) => set("answer", v)} rows={3} />
      ...
      <div className="flex items-center justify-between mt-2">
        <ConfirmButton label="삭제" onConfirm={async () => { await api(`/api/admin/list/faq/${item.id}`, { method: "DELETE" }); onDeleted(); }} />
...
export function FaqManager({ initial }: { initial: FaqItem[] }) {
  ...
  async function add() {
    const created = await api("/api/admin/list/faq", {
      method: "POST",
      body: JSON.stringify({ question: "", answer: "", visible: true }),
    });
  ...
  async function move(id: string, dir: -1 | 1) {
    ...
    await api("/api/admin/list/faq/reorder", { method: "POST", body: JSON.stringify({ orderedIds: copy.map((i) => i.id) }) });
  }

  return (
    <div>
      <h1 className="text-2xl font-bold mb-1">FAQ 관리</h1>
      <p className="text-sm text-neutral-500 mb-8">자주 묻는 질문과 답변을 추가·수정·삭제·순서 변경합니다.</p>
