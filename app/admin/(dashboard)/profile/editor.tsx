...
import { TextField, TextAreaField, FieldGroup } from "@/components/admin/fields";
...
export function ProfileEditor({ initial, initialPhilosophy }: { initial: Profile; initialPhilosophy: PhilosophySection }) {
  ...
  return (
    <div>
      ...
      <FieldGroup title="소개 탭 텍스트" hint="공개 화면 '프로필 > 소개' 탭에 나오는 글자입니다.">
        <TextAreaField
          label="짧은 자기소개"
          value={data.introShort}
          onChange={(v) => set("introShort", v)}
          rows={2}
          hint="⚠️ 현재 화면에는 표시되지 않습니다(참고용)."
        />
        <TextAreaField
          label="긴 자기소개"
          value={data.introLong}
          onChange={(v) => set("introLong", v)}
          rows={4}
          hint="⚠️ 현재 화면에는 표시되지 않습니다(참고용)."
        />
        <TextAreaField
          ...
          rows={2}
          hint="소개 탭 맨 위 큰 문구. Enter로 줄바꿈하면 그대로 반영됩니다."
        />
        <div className="rounded-md border border-neutral-800 p-3">
          <p className="text-sm font-medium mb-1">문단 · 키워드</p>
          <p className="text-xs text-neutral-500 mb-3">
            대표 문구 아래에 보이는 문단(앞 2개만 표시)과 키워드 태그입니다. ⚠️ &quot;핵심 철학 관리&quot; 메뉴에서도 이 데이터를 편집할 수 있으니, 되도록 한쪽에서만 수정하세요.
          </p>
          <TextListEditor
            label="문단"
            items={paragraphs}
